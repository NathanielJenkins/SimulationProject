//Packet simulation with 1 queue

import java.util.*; 

public  class PacketSimulationOneQueue { 
	...
	public static RouterSingleFIFONode rNode = new RouterSingleFIFONode (-1); 
	//public static RouterSinglePriorityNode rNode = new RouterSinglePriorityNode (-1); ; 
	...
	public static void init(){
		...
		meanSourceInterDepartureTime = 1.0/750; 
	...
	public static void processRouterArrival(Packet p){
		...
		if (debug){
			System.out.println("Processing a router Arrival:   \tPacket " + p.sequenceNumber + "   \tclock: " + clock);
		...
		//not idle
			...
			if(p.sequenceNumber > rNode.currentSeq){//inorder
				numberOfInOrderPackets++; 

				if (rNode.queue.size() < qSize * 2){
					if (debug){
						System.out.println("\tserver was processing, adding packet to single queue: " + p.sequenceNumber );
					}

					rNode.currentSeq = p.sequenceNumber;
					rNode.queue.add(p);

					if (rNode.queue.size() > qHMaxSize)	qHMaxSize = rNode.queue.size(); 

				} else {
					if (debug){
						System.out.println("\tSingle queue is full, size: " + rNode.queue.size() + " Dropped Packet " + p.sequenceNumber);
					}
				...
				if (rNode.queue.size() < qSize * 2){
					...
					rNode.queue.add(p); //add to the same queue
	...
	public static void processRouterDeparture(){
		...
		if (rNode.queue.peek() != null){
			Packet p = rNode.queue.remove();
			rNode.packetInService = p;
			scheduleRouterDeparture(p);

			if (debug){
				System.out.println("\tMoving into service from queue: " + p.sequenceNumber);
			}


		} else {

			if (rNode.queue.size() != 0 ){
				System.err.println("ERROR: Queue should be empty");
				System.exit(-1);
			}
			if (debug){
	...
	public static void scheduleRouterDeparture(Packet p){
		...
		//double transmitTimePerPacket = 0.1; 
	...
	public static void computeData(){
		...
		System.out.println("\tOut of order packets: \t" + numberOfOutOfOrderPackets);
		System.out.println("\tIn order packets: \t" + numberOfInOrderPackets);
		...
		System.out.println("\tPacket out of order rate: \t" + (double)numberOfOutOfOrderPackets/totalPackets);
		System.out.println("\tPacket in order rate:     \t" + (double)numberOfInOrderPackets/totalPackets);
		...
		System.out.println("\tMax single queue: " + qHMaxSize);
